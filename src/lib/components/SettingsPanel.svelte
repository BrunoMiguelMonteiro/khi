...
<script lang="ts">
  ...
  const metadataOptions: { key: keyof ExportConfig['metadata']; label: string }[] = [
  ...
  // Date format options
  const dateFormats: ExportConfig['dateFormat'][] = ['dd_mm_yyyy', 'dd_month_yyyy', 'iso8601'];
  ...
  // Sort options
  const sortOptions: SortPreference[] = ['title', 'author', 'date_last_read', 'highlight_count'];
  ...
  function getEnglishSortLabel(sort: SortPreference): string {
    switch (sort) {
      ...
      case 'date_last_read': return 'Last Read';
      case 'highlight_count': return 'Highlight Count';
      default: return sort;
    }
  }
    ...
    {#if activeTab === 'export'}
      ...
      >
        ...
        <section class="settings-section">
          ...
          <p class="section-description">Select which metadata to include in the exported files.</p>
          ...
          <div class="button-group">
            {#each viewModes as mode}
              <button
                ...
                class:active={uiPreferences.libraryViewMode === mode}
                onclick={() => handleViewModeChange(mode)}
                aria-pressed={uiPreferences.libraryViewMode === mode}
              >
                {#if mode === 'grid'}
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <section class="settings-section">
          ...
          <div class="select-wrapper">
            <select
              ...
              onchange={(e) => handleSortChange((e.target as HTMLSelectElement).value as SortPreference)}
              aria-label="Default library sort"
            >
              {#each sortOptions as sort}
                <option value={sort}>{getEnglishSortLabel(sort)}</option>
              {/each}
            </select>
          </div>
        </section>
